...
{
    public class Movies_playingModel : PageModel
    {
        private int pageSize = 6;

        private readonly IMovieService _service;

        public Movies_playingModel(IMovieService servie)
        {
            _service = servie;
        }
        ...
        public void OnGet(string searchstring, int pageIndex = 1)
        {
            ...
            int count = _service.GetMovies().Where(m => m.Title.Contains("active")).Count();

            var movies = _service.GetMovies(pageIndex, pageSize, count).Where(m => m.status.Contains("active"));
            var movie_trailer = _service.GetMovies().Where(m => m.status.Contains("trailer"));

            if (!string.IsNullOrEmpty(searchstring))
            {
                movies = movies.Where(m => m.Title.Contains(searchstring));
                count = _service.GetMovies().Where(m => m.Title.Contains(searchstring) && m.status.Contains("active")).Count();
            }
            var movietrailer = movie_trailer.OrderByDescending(m => m.MovieId).Take(5);
